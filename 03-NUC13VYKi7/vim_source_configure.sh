...
# prepare necessary deb package 
sudo apt install -y libncurses-dev libncurses5-dev python3-dev libgtk-3-dev \
	tcl tcl8.6-dev libx11-dev libxtst-dev libxt-dev libsm-dev libxpm-dev

vim_version=$(git tag  -l | tail -n 1 | sed 's#v##') 

./configure  --prefix=/usr/local/vim/${vim_version} \
--with-features=huge \
...
--enable-gui=gtk3 \
--with-lua-prefix=/usr/local/lua/5.4.6/ \
--with-x \
